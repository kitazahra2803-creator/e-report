...
<body class="min-h-screen relative">
    ...
    <div class="bg-[#7fc8c6] px-8 py-6 shadow relative z-10">
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="text-white text-sm">Admin Provinsi Jawa Barat</p>
            </div>
    ...
    <div class="relative z-10 flex justify-center py-10">
        <div class="w-[600px] bg-white/70 backdrop-blur-md rounded-2xl p-6 shadow-xl">
            ...
            <div class="space-y-3 text-sm">
                <div><b>Desa Pelapor</b><br><p class="bg-white/50 rounded-lg px-3 py-2 mt-1">{{ $report->desaRelasi->nama_desa ?? $report->desa ?? '-' }}</p></div>
                <div><b>Kecamatan</b><br><p class="bg-white/50 rounded-lg px-3 py-2 mt-1">{{ $report->kecamatan ?? '-' }}</p></div>
                <div><b>Kabupaten</b><br><p class="bg-white/50 rounded-lg px-3 py-2 mt-1">{{ $report->kabupaten ?? '-' }}</p></div>
                <div><b>Nama Pelapor</b><br><p class="bg-white/50 rounded-lg px-3 py-2 mt-1">{{ $report->user->name ?? '-' }}</p></div>
                ...
                <div>
                    ...
                    @if($report->kewenangan == 'Provinsi')
                        <span class="px-2 py-1 text-xs rounded-full bg-green-100 text-green-700 font-semibold">🏛️ Provinsi</span>
                    @else
                        <span class="px-2 py-1 text-xs rounded-full bg-gray-100 text-gray-600">🏘️ Lainnya</span>
                    @endif
                </div>
            ...
            @if($report->status != 'ditolak' && $report->status != 'selesai' && $report->kewenangan == 'Provinsi')
                ...
                <div class="space-y-3 text-sm">
                    ...
                    <div>
                        ...
                        <select id="kewenanganSelect" class="w-full mt-1 bg-white/70 rounded-lg px-3 py-2 border focus:outline-none focus:ring-2 focus:ring-blue-500">
                            <option value="Provinsi" {{ $report->kewenangan == 'Provinsi' ? 'selected' : '' }}>Provinsi</option>
                        </select>
                    </div>
                    ...
                    <div id="uploadFotoPerbaikan" class="hidden">
                        ...
                        <div id="previewFotoPerbaikan" class="mt-2 hidden">
                            <img id="previewPerbaikanImg" class="rounded-lg w-full max-h-32 object-cover">
                        </div>
                    </div>

                    <div>
                        <b>Keterangan/Catatan</b><br>
                        <textarea id="catatanProvinsi" rows="3" class="w-full mt-1 bg-white/70 rounded-lg px-3 py-2 border">{{ $report->catatan_provinsi ?? '' }}</textarea>
                    </div>
            ...
            <div class="mt-6">
                <a href="{{ route('admin-provinsi.dashboard') }}" class="block text-center px-4 py-2 bg-gray-300 hover:bg-gray-400 rounded-lg text-sm transition">Kembali ke Dashboard</a>
            </div>
        ...
        <div class="relative min-h-screen flex items-center justify-center p-4">
            <div class="relative bg-white rounded-2xl shadow-2xl max-w-md w-full mx-4 overflow-hidden">
                <div class="bg-gradient-to-r from-blue-500 to-blue-600 px-6 py-4 text-center">
                    ...
                    <p class="text-blue-100 text-sm">Prov. Jawa Barat</p>
                </div>
                <div class="px-6 py-6">
                    ...
                    <div class="flex gap-3 mt-6">
                        <button onclick="closeModalUpdate()" class="flex-1 px-4 py-2 bg-gray-300 rounded-lg">Cancel</button>
                        <button id="confirmUpdate" class="flex-1 px-4 py-2 bg-blue-500 text-white rounded-lg">Yes</button>
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="relative bg-white rounded-2xl shadow-2xl max-w-md w-full mx-4 overflow-hidden">
                ...
                <div class="px-6 py-6">
                    ...
                    <div class="mb-4">
                        ...
                        <textarea id="alasanTolakInput" rows="3" class="w-full px-3 py-2 border rounded-lg"></textarea>
                    </div>
                    <div class="flex gap-3">
                        <button onclick="closeModalTolak()" class="flex-1 px-4 py-2 bg-gray-300 rounded-lg">Batal</button>
                        <button id="confirmTolak" class="flex-1 px-4 py-2 bg-red-500 text-white rounded-lg">Tolak</button>
                    </div>
                </div>
            </div>
        </div>
    ...
    <script>
        function openModalUpdate() { document.getElementById('modalUpdate').classList.remove('hidden'); }
        function closeModalUpdate() { document.getElementById('modalUpdate').classList.add('hidden'); }
        function openModalTolak() { document.getElementById('modalTolak').classList.remove('hidden'); document.getElementById('alasanTolakInput').value = ''; }
        function closeModalTolak() { document.getElementById('modalTolak').classList.add('hidden'); }

        let statusSelect = document.getElementById('statusSelect');
        let uploadFotoDiv = document.getElementById('uploadFotoPerbaikan');
        if (statusSelect) {
            statusSelect.addEventListener('change', function() {
                if (this.value === 'selesai') uploadFotoDiv.classList.remove('hidden');
                else uploadFotoDiv.classList.add('hidden');
            });
        }
        ...
        function updateStatus(status, kewenangan = null, catatan = null, alasanTolak = null) {
            ...
            let fotoFile = document.getElementById('fotoPerbaikanInput');
            if (fotoFile && fotoFile.files[0]) formData.append('foto_perbaikan', fotoFile.files[0]);

            fetch(`/admin-provinsi/reports/{{ $report->id }}/status`, {
                method: 'POST', headers: { 'X-CSRF-TOKEN': '{{ csrf_token() }}' }, body: formData
            })
            .then(response => response.json())
            .then(data => { if (data.success) window.location.href = "{{ route('admin-provinsi.dashboard') }}"; else alert('Gagal: ' + data.message); })
            .catch(error => alert('Error: ' + error));
        }

        let btnUpdate = document.getElementById('btnUpdate'), confirmUpdate = document.getElementById('confirmUpdate');
        let btnTolak = document.getElementById('btnTolak'), confirmTolak = document.getElementById('confirmTolak');
        let kewenanganSelect = document.getElementById('kewenanganSelect'), catatanProvinsi = document.getElementById('catatanProvinsi');

        if (btnUpdate) btnUpdate.addEventListener('click', () => openModalUpdate());
        if (confirmUpdate) confirmUpdate.addEventListener('click', () => {
            let status = statusSelect ? statusSelect.value : 'menunggu';
            let kewenangan = kewenanganSelect ? kewenanganSelect.value : 'Provinsi';
            let catatan = catatanProvinsi ? catatanProvinsi.value : null;
            closeModalUpdate(); updateStatus(status, kewenangan, catatan);
        });
        if (btnTolak) btnTolak.addEventListener('click', () => openModalTolak());
        if (confirmTolak) confirmTolak.addEventListener('click', () => {
            let alasan = document.getElementById('alasanTolakInput').value;
